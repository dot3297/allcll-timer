/**
 * TodoStatsScreen — 할일 통계 화면
 *
 * ## 개요
 * 할일 완료 현황 및 통계를 보여주는 화면 컴포넌트.
 *
 * ## 주요 기능
 * - 뒤로가기 버튼: onBack 콜백으로 이전 화면 복귀
 *
 * ## 현재 상태 (TODO)
 * - [ ] 미완성 화면 — 추후 개발 예정
 * - [ ] 통계 데이터 표시 UI 구현 필요
 * - [ ] 서버 API 연동 필요
 */
// Back-arrow path (same as SettingsScreen / TodoScreen)
const PATH_BACK =
  "M0 1.13137L1.13137 0L6.56569 5.43431L12 0L13.1314 1.13137L6.56569 7.69706L0 1.13137Z";

type Props = { onBack: () => void };

export default function TodoStatsScreen({ onBack }: Props) {
  return (
    <div className="bg-[#262626] flex flex-col h-full w-full relative">
      {/* Status Bar */}
      ...
      {/* Nav bar */}
      <div className="flex items-center justify-between px-[12px] py-[16px] relative shrink-0 w-full">
        {/* 뒤로가기 */}
        <button
          type="button"
          onClick={onBack}
          aria-label="뒤로가기"
          className="size-[36px] flex items-center justify-center rounded-[8px] active:bg-[#333] transition-colors"
        >
        ...
        {/* 타이틀 — 중앙 */}
        <p className="absolute left-1/2 -translate-x-1/2 font-['Pretendard:Medium',sans-serif] text-[16px] leading-[24px] text-[#f9f9fa] text-center whitespace-nowrap top-[16px]">
          할일 통계
        </p>
